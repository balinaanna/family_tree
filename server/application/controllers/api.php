...
class Api extends CI_Controller {
	...
	public function get_tree() {
		$tree = $this->api_model->getTree($this->session->userdata('user_id'));
		echo json_encode($tree);
	}

	public function login() {
		$email = $this->input->post('email', TRUE);
		$pass = $this->input->post('pass') ? md5(md5($this->input->post('pass') )) : ($this->input->post('autologin') ? $this->input->post('autologin') : '');
		
		$json->action = "login";
		$json->status = "0";
		$json->message = "invalid email or password";
		if(!$email || !$pass) die(json_encode($json));
		
		$db_result = $this->api_model->getLogin($email, $pass);
		if ($db_result) {
			$json->action = "login";
			$json->status = "1";
			...
			$session_data = array('user_id' => $json->id, 'pass' => $pass, 'prof_id' => $json->prof_id);
