	...
	BaseView = Backbone.View.extend({
		...
		initialize : function() {
			...
			$('#navigator').on('mousemove', function() {
				$('#navigator').css('opacity', '0.7');
			});
			$('#navigator').on('mouseout', function() {
				$('#navigator').css('opacity', '0.3');
			});
			...
			for(var key in this.objects) {
				if(this.objects[key].info.id == data2.id)
		...
		create_node : function(data) {
			...
			var elems = {
				'parent' : {
					...
					posX : this.mouseX,
					posY : this.mouseY - Math.floor(this.nodeHeight / 2)
				},
				'child' : {
					width : this.imgPlusSize,
					height : this.imgPlusSize,
			...
			cube.add(this.texture('trash/polaroid.png', this.nodeWidth * 0.8, this.nodeHeight));
			// children[0]
           ...
           // var data = data2.tree;
			...
			if(i < 4) {
				...
				if(this.collection.get(id).toJSON().f_id){
					...
					console.log(this.collection.get(id).toJSON().f_id);
			...
			if(data[id].ch_ids) {
				for(var key in data[id].ch_ids) {
					...
					for(var k in this.objects) {
						if(this.objects[k].info.id == data[nodex.info.id].spouse_id)
							var spNodex = this.objects[k];
					}
					if(data[id].ch_ids.length != 1) {
						if(data[nodex.info.id].sex == "m") {
							if(key == 0) {
						...
						} else if(data[nodex.info.id].sex == "f") {
							if(key == 0) {
								chNode.position.set(nodex.position.x, nodex.position.y + 100 + this.nodeHeight, 0);
							} else {
								chNode.position.set(nodex.position.x - dx + 100, nodex.position.y + 100 + this.nodeHeight, 0);
