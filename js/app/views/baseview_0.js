define(['models/TreeNodeModel', 'collections/TreeCollection', 'models/TreeNodeModel1'],function(TreeModel, TreeCollection, TreeModel1){
    BaseView = Backbone.View.extend({
		...
		initialize: function(){
            ...
            //this.data2 = this._model.get("tree"); 
			...
			this.collection.fetch({url: '/data2.json',
    				success: $.proxy(function(collection) {
       						...
       						for(key in arr){console.log(arr[key].id);
       							
       							this.data[arr[key].id] = arr[key];
       						}
       						this.data2.tree = this.data;
			...
			if(i < 4) {
				...
				if(this.collection.get(id).toJSON().f_id) {
					...
					for (var k in this.objects){
                        for (var k2 in this.collection.get(f_id).toJSON().ch_ids){
                            if (this.objects[k].info.user_id == this.collection.get(f_id).toJSON().ch_ids[k2]) this.objects[k].father = f_node;
                        }
                    }
                    this.create_relation(nodex,f_node,"father","parent",f_id,i);
                    if (this.collection.get(f_id).toJSON().ch_ids.length == 1 || i > 2) f_node.position.set(nodex.position.x - (Math.pow((4 - i), 0.5)) * this.nodeWidth, (i - 1) * (-this.nodeHeight - 200), 0);
                    
                    if (this.collection.get(f_id).toJSON().ch_ids.length > 1 && i < 3){
				...
				if(this.collection.get(id).toJSON().m_id) {
                    ...
                    for (var k in this.objects){
                        for (var k2 in this.collection.get(m_id).toJSON().ch_ids){
                            if (this.objects[k].info.user_id == this.collection.get(m_id).toJSON().ch_ids[k2]) this.objects[k].mother = m_node;
                        }
                    }
					this.create_relation(nodex,m_node,"mother","parent",m_id,i);
                    if (this.collection.get(m_id).toJSON().ch_ids.length == 1 || i > 2) m_node.position.set(nodex.position.x + (Math.pow((4 - i), 0.5)) * this.nodeWidth, (i - 1) * (-this.nodeHeight - 200), 0);
					if (this.collection.get(m_id).toJSON().ch_ids.length > 1 && i < 3){
                        if (this.collection.get(id).toJSON().sex=="m"){
                        ...
                        if (this.collection.get(id).toJSON().sex=="f"){
                            ...
                            if (norm < needed) {
                                ...
                                var dx = needed/(this.collection.get(m_id).toJSON().ch_ids.length - 1);
                            }else{
                                m_node.position.set(nodex.position.x + norm, (i - 1) * (-this.nodeHeight - 200), 0);
                                if (i==2) this.width_spouse_for_m = norm;
                                var dx = norm/(this.collection.get(m_id).toJSON().ch_ids.length - 1);
                            }
                        }
                    }
                ...
                if(p_id && i < 3){
                    for (var k in this.collection.get(p_id).toJSON().ch_ids){
                        if (this.collection.get(p_id).toJSON().ch_ids[k] != id){
                            var ch_id = this.collection.get(p_id).toJSON().ch_ids[k];
        ...
        create_spouse: function(rootId, nodex){
            //var data2 = this.data2;
           // var data = data2.tree;
           //if(this.collection.get(rootId).toJSON().spouse_id == 0)return;
          // console.log(this.collection.get(rootId).toJSON().spouse_id);
            var id = this.collection.get(rootId).toJSON().spouse_id;
            if (this.collection.get(rootId).toJSON().sex == "m"){
                var dx = this.width_spouse_for_m + this.nodeWidth + 600;
                if (dx < this.chWidth[rootId]) dx = this.chWidth[rootId] 
            }else{
			...
			for (var key in this.objects){
			    if (this.objects[key].info.user_id == rootId) spouse_node = this.objects[key];
			}
        ...
        chLShift: {},
        chRShift: {},
        countChildren: function(rootId,id){
            //var data2 = this.data2;
            //var data = data2.tree;
            var sumWidth = 0;
            var w = this.nodeWidth + 150;
			if (this.collection.get(id))
            for (var key in this.collection.get(id).toJSON().ch_ids){                
                var ch_id = this.collection.get(id).toJSON().ch_ids[key];                
                var width = 0;
                if (key == 0){
                    var width = 0;
                } else {
                ...
                if (this.collection.get(ch_id))
                if (this.collection.get(ch_id).toJSON().spouse_id) {
                    width += w;
                    if (this.collection.get(ch_id).toJSON().sex == "m") {this.chSide[ch_id] = "r"} else {this.chSide[ch_id] = "l"} 
                }
                this.chWidth[ch_id] = width;
                
                this.countChildren(rootId,ch_id);
            }
            if (this.collection.get(ch_id))
            for (var key in this.collection.get(id).toJSON().ch_ids){                
                if (this.collection.get(id).toJSON().ch_ids.length > 1){
                    sumWidth += this.chWidth[this.collection.get(id).toJSON().ch_ids[key]];
                    if (this.collection.get(id).toJSON().sex == "m"){
                        if (key == 0){
                            if (this.chSide[this.collection.get(id).toJSON().ch_ids[key]] == "l") sumWidth -= this.chWidth[this.collection.get(id).toJSON().ch_ids[key]];
                            if (this.chRShift[this.collection.get(id).toJSON().ch_ids[key]]) sumWidth += this.chRShift[this.collection.get(id).toJSON().ch_ids[key]];
                        }
                        if (key>0 && key < this.collection.get(id).toJSON().ch_ids.length-1){
                            if (this.chLShift[this.collection.get(id).toJSON().ch_ids[key]]) sumWidth += this.chLShift[this.collection.get(id).toJSON().ch_ids[key]];
                            if (this.chRShift[this.collection.get(id).toJSON().ch_ids[key]]) sumWidth += this.chRShift[this.collection.get(id).toJSON().ch_ids[key]];
                        }
                        if (key == this.collection.get(id).toJSON().ch_ids.length-1){
                            if (this.chSide[this.collection.get(id).toJSON().ch_ids[key]] == "r") sumWidth -= this.chWidth[this.collection.get(id).toJSON().ch_ids[key]];
                            if (this.chLShift[this.collection.get(id).toJSON().ch_ids[key]]) sumWidth += this.chLShift[this.collection.get(id).toJSON().ch_ids[key]];
                        }
                    }
                    if (this.collection.get(id).toJSON().sex == "f"){
                ...
                if (this.collection.get(id).toJSON().ch_ids){
                    var fId = this.collection.get(id).toJSON().ch_ids[0];
                    var lId = this.collection.get(id).toJSON().ch_ids[this.collection.get(id).toJSON().ch_ids.length-1];
                    if (this.collection.get(id).toJSON().ch_ids.length > 1){
                        if (this.collection.get(id).toJSON().sex == "m") {
        ...
        createChildren: function(id, nodex) {
            ...
            var dx = 0;
            if (this.collection.get(id))
            if (this.collection.get(id).toJSON().ch_ids.length > 0){
                for (var key in this.collection.get(id).toJSON().ch_ids){
                    var ch_id = this.collection.get(id).toJSON().ch_ids[key];
                    if (this.collection.get(ch_id))
                    var chNode = this.create_node(this.collection.get(ch_id).toJSON());
                    console.log(ch_id);
                    dx += this.chWidth[ch_id];
                    for (var k in this.objects){
                        if (this.objects[k].info.user_id == this.collection.get(nodex.info.user_id).toJSON().spouse_id) var spNodex = this.objects[k];
                    }
                    
                    if (this.collection.get(id).toJSON().ch_ids.length != 1){
                        if (this.collection.get(nodex.info.user_id).toJSON().sex == "m"){
                            if (key == 0){
                                chNode.position.set(nodex.position.x, nodex.position.y + 200 + this.nodeHeight, 0);
                            }else{
                                if (this.chRShift[this.collection.get(id).toJSON().ch_ids[key-1]]) dx += this.chRShift[this.collection.get(id).toJSON().ch_ids[key-1]] - this.chWidth[this.collection.get(id).toJSON().ch_ids[key]];
                                if (this.chLShift[ch_id]) dx += this.chLShift[ch_id];
                                if (this.chSide[ch_id] == "r") dx -= this.chWidth[ch_id];
                                if (this.chSide[this.collection.get(id).toJSON().ch_ids[key-1]] == "r") dx += this.chWidth[this.collection.get(id).toJSON().ch_ids[key-1]];
                                chNode.position.set(nodex.position.x + dx, nodex.position.y + 200 + this.nodeHeight, 0);
                            }
                        } else if (this.collection.get(nodex.info.user_id).toJSON().sex == "f"){
                            if (key == 0){
                                chNode.position.set(nodex.position.x, nodex.position.y + 200 + this.nodeHeight, 0);
                            }else{
                                if (this.chLShift[this.collection.get(id).toJSON().ch_ids[key-1]]) dx += this.chLShift[this.collection.get(id).toJSON().ch_ids[key-1]] - this.chWidth[this.collection.get(id).toJSON().ch_ids[key]];
                                if (this.chRShift[ch_id]) dx += this.chRShift[ch_id];
                                if (this.chSide[ch_id] == "l") dx -= this.chWidth[ch_id];
                                if (this.chSide[this.collection.get(id).toJSON().ch_ids[key-1]] == "l") dx += this.chWidth[this.collection.get(id).toJSON().ch_ids[key-1]];
                                chNode.position.set(nodex.position.x - dx, nodex.position.y + 200 + this.nodeHeight, 0);
                            }
                        }
                        if (this.collection.get(id).toJSON().ch_ids.length == 2 && key == 1){
                    ...
                    if (this.collection.get(ch_id))
                    if (this.collection.get(ch_id).toJSON().spouse_id != 0){
                        var chSpId = this.collection.get(ch_id).toJSON().spouse_id;
                        var chSpNode = this.create_node(this.collection.get(chSpId).toJSON());
                        
                        if (this.collection.get(chSpId).toJSON().sex == "m"){
		...
		createTree: function(rootId){
			
			this.create_tree(rootId, 1);
            
            for (var key in this.objects){
                if (this.objects[key].info.user_id == rootId) var nodeX = this.objects[key];
            }
            this.countChildren(rootId, rootId);
            if(this.collection.get(rootId).toJSON().spouse_id != 0){
                this.create_spouse(rootId,nodeX);
            }
            console.log(this.chWidth);
            console.log(this.chLShift);
            console.log(this.chRShift);
            console.log(this.chSide);
            this.createChildren(rootId, nodeX);
            for (var key in this.objects){
                if (this.objects[key].father && this.objects[key].mother){
                    ...
                    if (this.collection.get(this.objects[key].info.user_id).toJSON().spouse_id){
                        for (var k in this.objects){
                            if (this.objects[k].info.user_id == this.collection.get(this.objects[key].info.user_id).toJSON().spouse_id) var spNode = this.objects[k];
                        }
